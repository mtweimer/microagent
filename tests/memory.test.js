...
test("structured memory recalls relevant turns by term overlap", () => {
  ...
  memory.addTurn({ role: "user", text: "Talked about Atomic Habits and Deep Work" });
  memory.addTurn({ role: "assistant", text: "Added books to list" });

  const result = memory.query("What did we say about Deep Work?", { topK: 1 });
  assert.equal(result.results.length, 1);
  assert.match(result.results[0].text, /Deep Work/);
});

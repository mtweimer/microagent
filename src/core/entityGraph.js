...
function extractEmailNames(email) {
...
function candidateNamesFromArtifact(envelope, artifacts) {
  const names = [];
  const agent = envelope?.agent;
  if (agent === "ms.outlook") {
    for (const msg of artifacts?.messages ?? []) {
    ...
    for (const row of [...(artifacts?.prioritized ?? []), ...(artifacts?.hits ?? []), ...(artifacts?.mentions ?? [])]) {
      names.push(row.from, row.teamName, row.channelName, row.chatTopic, row.bodyPreview);
    }
    for (const row of artifacts?.fallbackMatches ?? []) {
...
function summarizeArtifact(envelope, artifacts) {
  if (envelope?.agent === "ms.outlook") {
    if (artifacts?.subject) return `Email: ${artifacts.subject}`;
    if (Array.isArray(artifacts?.messages) && artifacts.messages[0]?.subject) {
      return `Email search: ${artifacts.messages[0].subject}`;
    }
  }
  if (envelope?.agent === "ms.calendar") {
    if (Array.isArray(artifacts?.events) && artifacts.events[0]?.subject) {
      return `Calendar: ${artifacts.events[0].subject}`;
    }
  }
  if (envelope?.agent === "ms.teams") {
    if (Array.isArray(artifacts?.hits) && artifacts.hits[0]?.teamName) {
      return `Teams: ${artifacts.hits[0].teamName}`;
    }
    if (Array.isArray(artifacts?.fallbackMatches) && artifacts.fallbackMatches[0]?.label) {
      return `Teams workspace: ${artifacts.fallbackMatches[0].label}`;
    }
  }
...
export class EntityGraph {
  ...
  initialize() {
    ...
    `);
    this.selectEntityByNormStmt = this.db.prepare(
  ...
  upsertEntity(name, options = {}) {
    ...
    let row = this.selectEntityByNormStmt.get(normalized) ?? this.selectEntityByAliasStmt.get(normalized);
    if (!row) {
  ...
  observeExecution(envelope, artifacts = {}) {
    this.initialize();
    const now = new Date().toISOString();
    const artifactId = String(
      artifacts?.id ??
        artifacts?.events?.[0]?.webLink ??
        artifacts?.messages?.[0]?.id ??
        artifacts?.hits?.[0]?.id ??
        artifacts?.prioritized?.[0]?.id ??
  ...
  lookup(name, limit = 10) {
    this.initialize();
    const normalized = normalize(name);
    if (!normalized) return { query: name, entities: [] };
    const entity = this.selectEntityByNormStmt.get(normalized) ?? this.selectEntityByAliasStmt.get(normalized);
    if (!entity) return { query: name, entities: [] };
    const mentions = this.lookupMentionsStmt.all(entity.entity_id, limit).map((row) => ({
  ...
  recent(limit = 10) {
    this.initialize();
    return this.recentEntitiesStmt.all(limit).map((row) => ({
      entityId: row.entity_id,

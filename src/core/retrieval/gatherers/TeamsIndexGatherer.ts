...
export class TeamsIndexGatherer implements Gatherer {
  ...
  async gather(plan: RetrievalPlan): Promise<RetrievedEvidence[]> {
    const searchQuery =
      plan.intent === "exact" && plan.entities.length > 0 && /\b(want|wanted|ask|asked|need|needed|request|requested)\b/i.test(plan.query)
        ? plan.entities.join(" ")
        : plan.query;
    ...
    const evidence: RetrievedEvidence[] = [
      ...messages.map((row) => ({
        ...
        provenance: { externalId: String(row.id ?? ""), locator: String(row.webUrl ?? ""), query: searchQuery, gatheredBy: this.name }
      })),
      ...fallbacks.map((row) => ({
    ...
    if (evidence.length === 0 && (result.coverage || result.limitations)) {
      const coverage = typeof result.coverage === "object" && result.coverage !== null ? (result.coverage as AnyRecord) : {};
      const limitations = Array.isArray(result.limitations) ? result.limitations.slice(0, 3).join("; ") : "";
      evidence.push({
        id: `teams-coverage:${plan.query.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`,
        source: this.name,
        sourceType: "teams-index" as const,
        title: "Teams scan coverage",
        snippet:
          `Teams scan coverage: chats=${coverage.chatsScanned ?? 0}/${coverage.chatMessagesScanned ?? 0}, ` +
          `channels=${coverage.channelsScanned ?? 0}/${coverage.channelMessagesScanned ?? 0}. ${limitations}`.trim(),
        raw: result,
        tokenCostEstimate: 36,
        entityRefs: ["Teams"],
        provenance: { query: searchQuery, gatheredBy: this.name }
      });
    }
